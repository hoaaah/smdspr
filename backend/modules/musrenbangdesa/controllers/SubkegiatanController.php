...
class SubkegiatanController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $proses = Proses::find()->where('tahun =:tahun', [':tahun' => DATE('Y')+1])->andWhere('input_phased=:input', [':input' => 3])->andWhere('kd_kelurahan=:kd_kelurahan', [':kd_kelurahan' => Yii::$app->user->identity->tperan->kd_kelurahan])->one();
    ...
    public function actionTerima($id)
    {
        ...
        $connection ->createCommand()
                    ->update('t_subkegiatan', ['input_status' => 2, 'status_phased' => 3], 'id='. $id)
                    ->execute();
        $model = $this->findModel($id);
    ...
    public function actionInfotolak($id)
    {
        ...
        IF($model->load(Yii::$app->request->post())){
            ...
            IF($model->save()){
                $connection = \Yii::$app->db;
                $connection ->createCommand()
                            ->update('t_subkegiatan', ['input_status' => 3, 'status_phased' => 3], 'id='. $id)
                            ->execute();
    ...
    public function actionTangguh($id)
    {
        $connection = \Yii::$app->db;
        $connection ->createCommand()
                    ->update('t_subkegiatan', ['input_status' => 4, 'status_phased' => 3], 'id='. $id)
                    ->execute();
        $model = $this->findModel($id);
    ...
    public function actionDraft($id)
    {
        ...
        $connection = \Yii::$app->db;
        $connection ->createCommand()
                    ->update('t_subkegiatan', ['input_status' => 1, 'status_phased' => 3], 'id='. $id)
                    ->execute();       
