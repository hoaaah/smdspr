...
use Yii;
use backend\modules\musrenbangkecamatan\models\Proses;
use backend\modules\musrenbangkecamatan\models\ProsesSearch;
use backend\modules\musrenbangkecamatan\models\SubkegiatanSearch;
use backend\modules\musrenbangkecamatan\models\SubkegiatanSearchproses;
...
class ProsesController extends Controller
{
    ...
    {
        $model = $this->findModel($id);
        if($model->kd_kecamatan <> Yii::$app->user->identity->tperan->kd_kecamatan){
    ...
    public function actionProses($id)
    {
        ...
        $ba = 'insert into t_ba_subkegiatan 
                ...
                FROM t_subkegiatan a WHERE a.input_phased < 4 AND a.status_phased = 4 AND a.input_status = 2 AND a.kd_kecamatan = '.Yii::$app->user->identity->tperan->kd_kecamatan;
        ...
        $connection ->createCommand()
                    ->update('t_subkegiatan', ['input_status' => 1, 'status_phased' => 5], ['and', 
                        ...
                        'input_phased <= 4', 
                        'input_status = 2', 'status_phased = 4'
                        ])
                    ->execute();  
        $connection ->createCommand()
