...
$('form#{$model->formName()}').on('beforeSubmit',function(e)
{
    ...
    $.post(
        ...
        .done(function(result){
            if(result == 1)
            {
                $("#myModalSub").modal('hide'); //hide modal after submit
                //$(\$form).trigger("reset"); //reset form to reuse it to input
                $.pjax.reload({container:'#sub-pjax{$model->Kd_Urusan}{$model->Kd_Bidang}{$model->Kd_Unit}'});
            }else
            {
                $("#message").html(result);
            }
...
$('form#{$model->formName()}').on('beforeSubmit',function(e)
{
    ...
    $.post(
        ...
        .done(function(result){
            if(result == 1)
            {
                $("#myModalSububah").modal('hide'); //hide modal after submit
                //$(\$form).trigger("reset"); //reset form to reuse it to input
                $.pjax.reload({container:'#sub-pjax{$model->Kd_Urusan}{$model->Kd_Bidang}{$model->Kd_Unit}'});
            }else
            {
