...
/* @var $model common\models\RenjaProgramCapaian */
/* @var $form yii\widgets\ActiveForm */
?>

<div class="renja-program-capaian-form">

    <?php $form = ActiveForm::begin(['id' => $model->formName()]); ?>

    <?= $form->field($model, 'no_indikator')->textInput() ?>
...
$('form#{$model->formName()}').on('beforeSubmit',function(e)
{
    ...
    $.post(
        ...
        .done(function(result){
            if(result == 1)
            {
                $("#myModalCapaianubah").modal('hide'); //hide modal after submit
                //$(\$form).trigger("reset"); //reset form to reuse it to input
                $.pjax.reload({container:'#capaian-pjax{$id}'});
            }else
            {
                $("#message").html(result);
            }
