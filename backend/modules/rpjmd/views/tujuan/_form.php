...
<div class="rpjmd-tujuan-form">
    ...
    <?php $form = ActiveForm::begin(['id' => /*$model->formName()*/'dynamic-form' ,'options' => ['enctype' => 'multipart/form-data']]); ?>

    <?php echo $model->isNewRecord ? '' : $form->field($model, 'No_Tujuan')->textInput() ?>
    ...
    $.post(
        ...
        .done(function(result){
            if(result == 1)
            {
                ...
                //$(\$form).trigger("reset"); //reset form to reuse it to input
                $.pjax.reload({container:'#tujuan-pjax{$model->ID_Tahun}{$model->No_Misi}'});
            }else
            {
                $("#message").html(result);
            }
