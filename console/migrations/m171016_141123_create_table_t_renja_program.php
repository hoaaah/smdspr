...
class m171016_141123_create_table_t_renja_program extends Migration
{
    public function safeUp()
    {
        ...
        $this->createTable('{{%t_renja_program}}', [
        ...
        $this->createIndex('tahun', '{{%t_renja_program}}', ['tahun','kd_urusan','kd_bidang','kd_unit','kd_sub','no_skpdMisi','no_skpdTujuan','no_skpdSasaran','no_renjaSas','no_renjaProg','id_renprog'], true);
        $this->createIndex('t_renja_program_ibfk_8', '{{%t_renja_program}}', ['rkpd_program_id','id_tahun'], true);

        $this->addForeignKey('t_renja_program_ibfk_1', '{{%t_renja_program}}', 'user_id', '{{%user}}', 'id');
        $this->addForeignKey('t_renja_program_ibfk_3', '{{%t_renja_program}}', ['kd_urusan','kd_bidang','kd_unit','kd_sub'], '{{%r_sub_unit}}', ['Kd_Urusan','Kd_Bidang','Kd_Unit','Kd_Sub']);
        $this->addForeignKey('t_renja_program_ibfk_5', '{{%t_renja_program}}', ['kd_urusan','kd_bidang','kd_unit'], '{{%r_unit}}', ['Kd_Urusan','Kd_Bidang','Kd_Unit']);
    }

    public function safeDown()
    {
        $this->dropTable('{{%t_renja_program}}');
    }
}
