...
$this->title = 'Verifikasi Data Usulan '.$tahun;
$this->params['breadcrumbs'][] = ['label' => 'Musrenbang Desa/Kelurahan', 'url' => ['subkegiatan/']];
    ...
    <div class="col-xs-12">
        <div class="box">
            <div class="box-header">
                <h3 class="box-title"><?= "Kecamatan ".Yii::$app->user->identity->tperan->kecamatan->kecamatan." - Kelurahan/Desa ".Yii::$app->user->identity->tperan->kelurahan->desa.' - RW '.Yii::$app->user->identity->tperan->rw ?></h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">    
                <?php Pjax::begin(); ?>    
                <?= GridView::widget([
                        ...
                        'columns' => [
                            ...
                            // 'harga_satuan',
                            [
                                'attribute' => 'status_phased',
                                'label' => 'Tingkat Bahas',
                                'value' => 'statusPhased.keterangan'
                            ],                          
                            [
                                ...
                                'format' => 'raw',
                                'value'=> function($model){
                                    IF($model->status_phased > 3){
                                        $model->status_phased = 2;
                                        return '<span class="label label-success>Diterima</span>';
                                    }ELSE{
                                        return '<span class="label label-danger>'.$model->inputStatus->keterangan.'</span>';    
                                    }
                            ...
                            [
                                ...
                                'value' => function ($model, $image) use ($jadwal, $proses){
                                    return Html::a('+++', 
                                    ['subkegiatan/detail', 'id' => $model->id],
                                    [
                                        'id' => 'detail-'.$model->id,
                                        'class' => 'btn btn-xs btn-default detail-closed'
                                        // 'onclick'=>'detail(this,'.$model->id.');return false;'
                                    ]);
